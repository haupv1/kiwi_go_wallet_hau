...
type Wallet struct {
	...
	chainClient        *chain.RPCClient  // a client connection to  bitcoin RPC server
...
func (w *Wallet) Start() {
	...
	go w.txCreator()
	go w.walletLocker()
	fmt.Println("MR.HAU 's WALLET HAS STARTED")
}
...
func (w *Wallet) NextAccount(name string) (uint32, error) {
	fmt.Println("enter function NextAccount")
	account, err := w.Manager.NewAccount(name)
	if err != nil {
		fmt.Println("error when create new account")
		return 0, err
	}
	...
	fmt.Println("function NextAccount create new account successfully")
	return account, nil
}
...
func (w *Wallet) NewAddress(account uint32) (btcutil.Address, error) {
	...
	if err != nil {
		fmt.Println("error when create new address")
		return nil, err
	}

	// Request updates from btcd for new transactions sent to this address.
	fmt.Println("come to request to update from btcd")
	...
	if chainClient != nil {
		...
		if err != nil {
			fmt.Println("error when notify received with chainClient")
			return nil, err
		}
	}
	fmt.Println("come to set account property")
	props, err := w.Manager.AccountProperties(account)
	if err != nil {
	...
	fmt.Println("create address sucessfully")
...
func Open(db walletdb.DB, pubPass []byte, cbs *waddrmgr.OpenCallbacks, params *chaincfg.Params) (*Wallet, error) {
	...
	lastAccount, err := addrMgr.LastAccount()
	if err!=nil {
		fmt.Println("error when want to access last account")
	}
	fmt.Println("show the last account name",lastAccount)
