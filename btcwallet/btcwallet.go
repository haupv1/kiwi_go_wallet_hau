...
func main() {
	...
	if err := walletMain(); err != nil {
		fmt.Println("error when mr.Hau want to create wallet")
		os.Exit(1)
	}
...
func walletMain() error {
	...
	fmt.Println("Mr.HAU want to create wallet")
	...
	if !cfg.NoInitialLoad {
		...
		myWallet, err = loader.OpenExistingWallet([]byte(cfg.WalletPass), true)
		fmt.Println("loaded MR.HAU 's wallet database")
		if err != nil {
			log.Error(err)
			fmt.Println("error when load MR.HAU wallet database")
			return err
		}
		fmt.Println(myWallet.Accounts())
		fmt.Println("create new address")
		addr,_ := myWallet.NewAddress(0)
		fmt.Println("address is ",addr)
		//myWallet.NextAccount("haupv1@£")
	}
